...
export class ProductService {
  ...
  getProducts(filter: ProductFilter = undefined): Observable<Product[]> {
    ...
    const order = '_sort=publishedDate&_order=DESC';
    ...
    {      
      if (filter.text) {        
        textSearch = `&q=${filter.text}`;
      }
      if (filter.category && filter.category != '0') {
        categorySearch = `&category.id=${filter.category}`;
      }
      if (filter.state) {
        stateSearch = `&state=${filter.state}`;
      }
      if (filter.minPrice) {
        minPriceSearch = `&price_gte=${filter.minPrice}`;
      }
      if (filter.maxPrice) {
        maxPriceSearch = `&price_lte=${filter.maxPrice}`;
      }
      if (filter.userId) {
        userIdSearch = `&seller.id=${filter.userId}`;
      }
    }

    return this._http
      .get(`${this._backendUri}/products?${order}${textSearch}${categorySearch}${stateSearch}${minPriceSearch}${maxPriceSearch}${userIdSearch}`)
  ...
  setFavoriteProduct(productId: number, favorite: boolean): Observable<Product> {
    const body: any = { 'favorite': !favorite };
    return this._http
      .patch(`${this._backendUri}/products/${productId}`, body)
      .map((data: Response): Product => Product.fromJson(data.json()));
  }
